class Facility:
    def __init__(self, DB, id, name, type, address, email, phone, new=False):
        ...
        if new:
            self.id = str(self.DB.add_facility(
                self.name, self.type, self.address, self.email, self.phone))
    ...
    def push(self):
        self.DB.update_facility(
            self.id, self.name, self.type, self.address, self.email, self.phone)
...
class FacilityManagement:
    ...
    def pull(self):
        ...
        for i in self.DB.get_facilities():
            f.append(Facility(
                self.DB, i['id'], i['name'], i['type'], i['address'], i['email'], i['phone']))
        self.f = f

    def add_facility(self, name, type, address, email, phone):
        self.f.append(Facility(self.DB, '-1', name, type,
                      address, email, phone, new=True))

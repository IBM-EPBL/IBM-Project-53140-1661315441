class DBManagerProxy():
    ...
    def add_user(self, username, password, name, role, email, phone):
        print("Adding User: " + str({'username': username, 'password': password,
    ...
    def update_user(self, username, password, newname, newrole, newemail, newphone):
        print("Updating User: " + str({'username': username, 'password': password,
              'name': newname, 'role': newrole, 'email': newemail, 'phone': newphone}))
        for i in self.users:
            if i['username'] == username and i['password'] == password:
                i['name'] = newname
                i['role'] = newrole
    ...
    def change_password(self, username, oldpassword, newpassword):
        print("Changing Password: " + str({'username': username,
    ...
    def add_facility(self, name, type, address, email, phone):
        print("Adding Facility: " + str({'name': name, 'type': type,
              'address': address, 'email': email, 'phone': phone}))
        self.facilities.append({'id': str(len(self.facilities)), 'name': name,
    ...
    def update_facility(self, id, newname, newtype, newaddress, newemail, newphone):
        print("Updating Facility: " + str({'id': id, 'name': newname, 'type': newtype,
              'address': newaddress, 'email': newemail, 'phone': newphone}))
        for i in self.facilities:
            if i['id'] == id:
                i['name'] = newname
                i['type'] = newtype
    ...
    def add_item(self, name, type, price, quantity, facility):
        print("Adding Item: " + str({'name': name, 'type': type,
              'price': price, 'quantity': quantity, 'facility': facility}))
        self.items.append({'id': str(len(self.items)), 'name': name, 'type': type,
                          'price': price, 'quantity': quantity, 'facility': facility})
    ...
    def update_item(self, id, newname, newtype, newprice, newquantity, newfacility):
        print("Updating Item: " + str({'id': id, 'name': newname, 'type': newtype,
              'price': newprice, 'quantity': newquantity, 'facility': newfacility}))
        for i in self.items:
            if i['id'] == id:
                i['name'] = newname
                i['type'] = newtype
